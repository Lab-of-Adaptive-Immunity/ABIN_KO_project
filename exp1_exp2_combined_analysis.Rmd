...
Performing some supplementary data analysis using bulk RNA seq data from Sarka's experiment.
...
lplot1 <- ggplot(fold_change_integ, aes(x = log2FoldChange.exp1, y = log2FoldChange.exp2, label = label)) +
  ...
  geom_text(data = lm_data_fold, aes(x = xpos, y = ypos, hjust = hjust, vjust = vjust, label = label), col = 'blue') +
  geom_point(data = fold_change_integ %>% dplyr::filter(label != ''), pch = 21, size = 4, colour = 'purple') +
  ...
  annotate('text',  
           ...
           label = paste0('y = ', round(lmExps_fold$coefficients[2], 3), ' + ',  round(lmExps_fold$coefficients[1], 3)),
           hjust = 0) +
  geom_abline(slope = lmExps_fold$coefficients[2], intercept = lmExps_fold$coefficients[1], col = 'blue') + 
  geom_abline(slope = 1, linetype = 'dashed', col = 'firebrick4') + 
...
```{r}
tiff('exp1_exp2_linear_plot.tiff', units = 'in', width = 7.5, height = 5.5, res = 150)
print(lplot1)
dev.off()
```
...
```{r}
tiff('exp1_exp2_linear_plot_blank.tiff', units = 'in', width = 7.5, height = 5.5, res = 150)
print(lplot1)
dev.off()
```
...
```{r}
write.csv(fc_upreg_set_exp1_exp2[c('EnsemblID', 'symbol')],'GSEA_tables/exp1_exp2_up_regulated_genes.csv')
write.csv(fc_downreg_set_exp1_exp2[c('EnsemblID', 'symbol')],'GSEA_tables/exp1_exp2_down_regulated_genes.csv')
```
...
GSEA1_pos <- plotEnrichment(GSEA.genes[['Activated_in_GTKO']],
               ranked_all) + 
  annotate("text", label = paste('pval = ', round(fgsea_all$pval[1], 6)), x = xcoord, y = ycoord1, size = 7, hjust = 0, col = 'firebrick2') +
...
```{r}
tiff('exp1_exp2_GSEA_upreg.tiff', units = 'in', width = 7.5, height = 4.5, res = 150)
print(GSEA1_pos)
dev.off()
```
...
```{r}
tiff('exp1_exp2_GSEA_upreg_blank.tiff', units = 'in', width = 7.5, height = 4.5, res = 150)
print(GSEA1_pos_blank)
dev.off()
```
...
GSEA1_neg <- plotEnrichment(GSEA.genes[['Deactivated_in_GTKO']], ranked_all) + 
  annotate("text", label = paste('pval = ', round(fgsea_all$pval[2], 6)), x = xcoord, y = ycoord1, size = 7, hjust = 0, col = 'firebrick2') +
...
```{r}
tiff('exp1_exp2_GSEA_downreg.tiff', units = 'in', width = 7.5, height = 4.5, res = 150)
print(GSEA1_neg)
dev.off()
```
...
```{r}
tiff('exp1_exp2_GSEA_downreg_blank.tiff', units = 'in', width = 7.5, height = 4.5, res = 150)
print(GSEA2_pos_blank)
dev.off()
```
...
tiff('exp2_GSEA_Select_heatmap.tiff', units = 'in', width = 4.5, height = 9.5, res = 150)
